...
import { RToken } from "register-abis/types"
import {
    blockTimestampAtom,
    isBorrowModalVisibleAtom,
    isRepayModalVisibleAtom,
    ...
    const columns = useMemo(
        () => [
            ...
            },
            {
                Header: t`Available`,
                accessor: "balance",
                Cell: (cell: any) => <Text>{(cell.cell.value as number).toFixed(2)}</Text>,
            },
            {
                Header: t`Liquidity`,
                accessor: "liquidity",
            ...
            },
            {
                Header: t`Total APY`,
                accessor: "borrowApy",
                Cell: (cell: any) => <Text sx={{ color: "red" }}>{(cell.cell.value as number).toFixed(2)}%</Text>,
            },
            {
                Header: "",
                accessor: "maxBorrow",
            ...
            <Box {...props}>
                ...
                {data.length ? (
                    ...
                    <Box
                        ...
                        p={6}
                    >
                        <Text>"No lending markets available. Something has probably gone wrong."</Text>
                    </Box>
                )}
            </Box>
        </>
...
const Borrow = () => {
    ...
    return (
        <Container>
            <ContentHead title={`Borrow Assets`} subtitle={t``} mb={7} ml={5} />
