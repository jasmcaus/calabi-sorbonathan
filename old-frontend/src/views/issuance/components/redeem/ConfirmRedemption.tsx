...
const ConfirmRedemption = ({ onClose }: { onClose: () => void }) => {
    ...
    return (
        <TransactionModal
            ...
            confirmLabel={t`Begin redemption of ${formatCurrency(Number(amount))} ${rToken?.symbol ?? ""}`}
            buildApprovals={buildApproval}
            onClose={handleClose}
            onChange={(signing) => setSigning(signing)}
        >
            <RedeemInput compact disabled={signing} />
            <CollateralDistribution mt={3} collaterals={rToken?.collaterals ?? []} quantities={collateralQuantities} />
        </TransactionModal>
    )
}
