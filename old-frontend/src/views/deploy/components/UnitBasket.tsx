...
const UnitBasket = ({ data, readOnly, unit, ...props }: UnitBasketProps) => {
    ...
    const handleRemove = (index: number) => {
        ...
        updateBasket([
            unit,
            {
                ...data,
                distribution,
                collaterals: [...data.collaterals.slice(0, index), ...data.collaterals.slice(index + 1)],
            },
        ])
    }
    ...
    const handleDistribution = (index: number, value: string) => {
        updateBasket([
            ...
            {
                ...data,
                distribution: [...data.distribution.slice(0, index), value, ...data.distribution.slice(index + 1)],
            },
        ])
    }
        ...
        <Box {...props}>
            {!readOnly && (
                <>
                    <Flex variant="layout.verticalAlign">
                        <Text sx={{ fontWeight: 500 }}>{unit} - "Basket scale"</Text>
                    ...
                    <Flex variant="layout.verticalAlign">
                        ...
                        <Text ml="auto" sx={{ color: totalDistribution !== 100 ? "danger" : "text" }}>
                            {totalDistribution}%
                        </Text>
                    </Flex>
                </>
            ...
            {data.collaterals.map((collateral, index) => (
                <Flex key={collateral.address} variant="layout.verticalAlign" mt={3}>
                    ...
                    <Text>%</Text>
                    {!readOnly && (
                        <IconButton ml={2} sx={{ cursor: "pointer" }} onClick={() => handleRemove(index)}>
