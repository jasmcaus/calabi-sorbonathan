...
const tradesQuery = gql`
    query Trades($id: String!, $time: Int!) {
        current: trades(where: { endAt_gt: $time, rToken: $id }, orderBy: startedAt, orderDirection: desc) {
            id
            amount
            auctionId
            buying
        ...
        ended: trades(where: { endAt_lte: $time, rToken: $id }, orderBy: startedAt, orderDirection: desc) {
            id
            amount
            auctionId
            buying
                        ...
                        <Box as="tr" {...headerGroup.getHeaderGroupProps()}>
                            {headerGroup.headers.map((column: any) => (
                                <Box
                                    as="th"
                                    variant="styles.th"
                                    {...column.getHeaderProps(sorting ? column.getSortByToggleProps() : undefined)}
                                >
                                    <Flex pb={compact ? 2 : 0} variant="layout.verticalAlign">
                                        <Box sx={{ mr: 1, flex: 1 }}>{column.render("Header")}</Box>
                                        {sorting && column.isSorted && (column.isSortedDesc ? "." : "up")}
                                    </Flex>
                                </Box>
                            ))}
                        </Box>
                        ...
                        (
                            ...
                            return (
                                <React.Fragment key={key}>
                                    ...
                                    >
                                        {row.cells.map(
                                            (cell: Cell & Partial<UseGroupByCellProps<D>>, cellIndex: number) => (
                                                <Box
                                                    as="td"
                                                    sx={{
                                                        ...
                                                        borderTopRightRadius:
                                                            compact && !index && cellIndex === row.cells.length - 1
                                                                ? `${borderRadius.boxes}px !important`
                                                                : undefined,
                                                        ...
                                                        borderBottomRightRadius:
                                                            compact && last && cellIndex === row.cells.length - 1
                                                                ? `${borderRadius.boxes}px !important`
                                                                : undefined,
                                                    }}
                                                    variant="styles.td"
