...
const ApprovalTransactions = ({
    ...
    const [signed, failedTx] = useMemo(() => {
        ...
        const allSigned =
            !!txState.length &&
            txState.every((tx) => tx.status === TRANSACTION_STATUS.MINING || tx.status === TRANSACTION_STATUS.CONFIRMED)
    ...
    return (
        <>
            {!!failedTx && (
                <TransactionError title="Transaction failed" subtitle={failedTx.description} onClose={handleRetry} />
            )}
            <Box mt={4}>
                <LoadingButton loading={!!signing} text={title} onClick={handleApprove} sx={{ width: "100%" }} />
