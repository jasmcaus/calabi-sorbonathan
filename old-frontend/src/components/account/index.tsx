...
const ErrorWrapper = ({ chainId, children, isValid }: { isValid: boolean; chainId?: number; children: ReactNode }) =>
    isValid ? (
        <>{children}</>
    ) : (
            ...
            content={
                <Card sx={{ width: 320 }}>
                    ...
                    <Text variant="legend" sx={{ fontSize: 1 }}>
                        " Delta only supports Delta Saga Testnet currently. Change your network in the connected wallet.
    ...
    return (
        <>
            {!account ? (
                ...
                >
                    <Text sx={{ display: ["none", "initial"] }}>"Connect"</Text>
                ...
                <ErrorWrapper isValid={!isInvalid} chainId={chainId}>
                    <Container onClick={() => setVisible(true)}>
                        ...
                        <Blockies className="blockies" color="#dfe" bgColor="#aaa" seed={account} size={25} scale={1} />
