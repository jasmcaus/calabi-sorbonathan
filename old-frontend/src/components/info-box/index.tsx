...
export const ContentHead = ({ title, subtitle, fontSize, ...props }: any) => (
    <Box {...props}>
        ...
        {!fontSize && <Text sx={{ display: "block", marginBottom: 1, fontWeight: 500, fontSize: 4 }}>{title}</Text>}

        {!!subtitle && (
            <Text as="p" pr={3} sx={{ maxWidth: 620, fontWeight: 300, fontSize: 3 }} variant="legend">
                {subtitle}
            </Text>
        )}
    </Box>
