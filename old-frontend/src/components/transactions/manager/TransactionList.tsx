        ...
        case TRANSACTION_STATUS.CONFIRMED:
            return (
                <Flex variant="layout.verticalAlign">
                    <Check size={18} />
                    <Text ml={2} sx={{ display: ["none", "flex"] }}>
                        "Confirmed, Block {tx.confirmedAt}"
                    </Text>
                </Flex>
            )
...
const getTxDescription = (tx: TransactionState) => {
    ...
    if (tx.call.method === "setupGovernance") {
        return (
            <RouterLink style={{ color: "var(--theme-ui-colors-lightText)" }} to={`${ROUTES.GOVERNANCE_INFO}/${tx.id}`}>
                {tx.description}
            </RouterLink>
        )
    }
            ...
            {Object.keys(txs).map((day) => (
                <Box key={day} mb={4}>
                    ...
                    {txs[day].map((tx) => (
                        ...
                        >
                            <Flex sx={{ overflow: "hidden", alignItems: "center" }}>{getTxDescription(tx)}</Flex>
                            <Flex sx={{ overflow: "hidden", display: ["none", "flex"] }} variant="layout.verticalAlign">
