...
const AreaChart = ({ data, title, heading, currentRange, timeRange, onRangeChange, ...props }: Props) => {
    const gain = useMemo(() => {
        if (data && data.length > 1) {
            return Math.floor(((data[data.length - 1].value - data[0].value) / (data[0].value || 1)) * 100)
        }
        ...
        <Box {...props}>
            ...
            {data && !data.length && (
                <Box my={6} sx={{ textAlign: "center", height: 60 }}>
                    <Text variant="legend">"No data"</Text>
                </Box>
            )}
            {!data && (
                <Box my={6} sx={{ textAlign: "center", height: 60 }}>
