...
import { Facade } from "register-abis/types"
...
const getTokens = (reserveToken: ReserveToken): [string, number][] => {
    const addresses: [string, number][] = [
        ...
        ...reserveToken.collaterals.map((token): [string, number] => [token.address, token.decimals]),
    ]
...
const TokensBalanceUpdater = () => {
    ...
    const balances = useTokensBalance(rToken && account ? getTokens(rToken) : [], account)
...
const TokensAllowanceUpdater = () => {
    ...
    const allowances = useTokensAllowance(reserveToken && account ? getTokenAllowances(reserveToken) : [], account)
