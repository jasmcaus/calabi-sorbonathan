...
async function main() {
  ...
  const { collateral: daiCollateral } = await hre.run(
    "deploy-fiat-collateral",
    {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(assetCollDeployments, null, 2)
  );
  ...
  const { collateral: usdcCollateral } = await hre.run(
    "deploy-fiat-collateral",
    {
      fallbackPrice: fp("1").toString(),
  ...
  const { collateral: usdtCollateral } = await hre.run(
    "deploy-fiat-collateral",
    {
      fallbackPrice: fp("1").toString(),
  ...
  const { collateral: usdpCollateral } = await hre.run(
    "deploy-fiat-collateral",
    {
      fallbackPrice: fp("1").toString(),
  ...
  const { collateral: tusdCollateral } = await hre.run(
    "deploy-fiat-collateral",
    {
      fallbackPrice: fp("1").toString(),
  ...
  const { collateral: busdCollateral } = await hre.run(
    "deploy-fiat-collateral",
    {
      fallbackPrice: fp("1").toString(),
  ...
  const { collateral: aDaiCollateral } = await hre.run(
    "deploy-atoken-fiat-collateral",
    {
  ...
  const { collateral: aUsdcCollateral } = await hre.run(
    "deploy-atoken-fiat-collateral",
    {
      fallbackPrice: fallbackPrice.toString(),
      priceFeed: networkConfig[chainId].chainlinkFeeds.USDC,
  ...
  const { collateral: aUsdtCollateral } = await hre.run(
    "deploy-atoken-fiat-collateral",
    {
      fallbackPrice: fallbackPrice.toString(),
      priceFeed: networkConfig[chainId].chainlinkFeeds.USDT,
  ...
  const { collateral: aBusdCollateral } = await hre.run(
    "deploy-atoken-fiat-collateral",
    {
      fallbackPrice: fallbackPrice.toString(),
      priceFeed: networkConfig[chainId].chainlinkFeeds.BUSD,
  ...
  const { collateral: aUsdpCollateral } = await hre.run(
    "deploy-atoken-fiat-collateral",
    {
      fallbackPrice: fallbackPrice.toString(),
      priceFeed: networkConfig[chainId].chainlinkFeeds.USDP,
  ...
  const { collateral: cDaiCollateral } = await hre.run(
    "deploy-ctoken-fiat-collateral",
    {
      fallbackPrice: fallbackPrice.toString(),
      priceFeed: networkConfig[chainId].chainlinkFeeds.DAI,
      oracleError: fp("0.0025").toString(), // 0.25%
  ...
  const { collateral: cUsdcCollateral } = await hre.run(
    "deploy-ctoken-fiat-collateral",
    {
      fallbackPrice: fallbackPrice.toString(),
      priceFeed: networkConfig[chainId].chainlinkFeeds.USDC,
  ...
  const { collateral: cUsdtCollateral } = await hre.run(
    "deploy-ctoken-fiat-collateral",
    {
      fallbackPrice: fallbackPrice.toString(),
      priceFeed: networkConfig[chainId].chainlinkFeeds.USDT,
  ...
  const { collateral: cUsdpCollateral } = await hre.run(
    "deploy-ctoken-fiat-collateral",
    {
      fallbackPrice: fallbackPrice.toString(),
      priceFeed: networkConfig[chainId].chainlinkFeeds.USDP,
  ...
  fallbackPrice = (
    await getCurrentPrice(networkConfig[chainId].chainlinkFeeds.BTC)
  )
    .mul(await cToken.exchangeRateStored())
    .div(bn("1e18"));
  ...
  const { collateral: cWBTCCollateral } = await hre.run(
  ...
  fallbackPrice = (
    await getCurrentPrice(networkConfig[chainId].chainlinkFeeds.ETH)
  )
    .mul(await cToken.exchangeRateStored())
    .div(bn("1e28"));
  ...
  const { collateral: wBTCCollateral } = await hre.run(
    "deploy-nonfiat-collateral",
    {
      fallbackPrice: (
        await getCurrentPrice(networkConfig[chainId].chainlinkFeeds.BTC)
      ).toString(),
  ...
  const { collateral: wETHCollateral } = await hre.run(
    "deploy-selfreferential-collateral",
    {
      fallbackPrice: (
        await getCurrentPrice(networkConfig[chainId].chainlinkFeeds.ETH)
      ).toString(),
  ...
  const { collateral: eurtCollateral } = await hre.run(
    "deploy-eurfiat-collateral",
    {
      fallbackPrice: (
        await getCurrentPrice(networkConfig[chainId].chainlinkFeeds.EURT)
      ).toString(),
