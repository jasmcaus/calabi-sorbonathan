...
async function main() {
  ...
  if (fileExists(rTokenDeploymentFilename)) {
    throw new Error(
      `${rTokenDeploymentFilename} exists; I won't overwrite it.`
    );
  }
  ...
  if (!deployments.facadeWrite) {
    throw new Error(
      `Missing FacadeWrite address in network ${hre.network.name}`
    );
  } else if (!(await isValidContract(hre, deployments.facadeWrite))) {
    throw new Error(
      `FacadeWrite contract not found in network ${hre.network.name}`
    );
  }
