...
async function main() {
  ...
  const rTokenDeploymentFilename = getRTokenDeploymentFilename(
    chainId,
    RTOKEN_NAME
  );
  const rTokenDeployments = <IRTokenDeployments>(
    getDeploymentFile(rTokenDeploymentFilename)
  );
  ...
  if (
    ...
    !(await isValidContract(hre, rTokenDeployments.components.rToken))
  ) {
    throw new Error(`RToken contract not found in network ${hre.network.name}`);
  } else if (
    !(await isValidContract(hre, rTokenDeployments.components.stRSR))
  ) {
  ...
  const main: MainP1 = <MainP1>(
    await ethers.getContractAt("MainP1", await rToken.main())
  );
  const stRSR: StRSRP1 = <StRSRP1>(
    await ethers.getContractAt("StRSRP1", await main.stRSR())
  );
  ...
  const governanceAddr = expectInReceipt(receipt, "GovernanceCreated").args
    .governance;
  const timelockAddr = expectInReceipt(receipt, "GovernanceCreated").args
    .timelock;
  ...
  fs.writeFileSync(
    rTokenDeploymentFilename,
    JSON.stringify(rTokenDeployments, null, 2)
  );
