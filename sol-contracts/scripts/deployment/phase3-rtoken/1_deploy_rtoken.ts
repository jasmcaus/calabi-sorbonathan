...
async function main() {
  ...
  const rTokenSetup: IRTokenSetup = {
    ...
    primaryBasket: [
      ...
      assetCollDeployments.collateral.USDT as string,
  ...
  for (const assetAddr of rTokenSetup.assets) {
    if (!(await isValidContract(hre, assetAddr))) {
      throw new Error(
        `Asset contract not found in network ${hre.network.name}`
      );
    }
  }
  ...
  for (const collAddr of allCollateral) {
    if (!(await isValidContract(hre, collAddr))) {
      throw new Error(
        `Collateral contract not found in network ${hre.network.name}`
      );
    }
  }
  ...
  // Get Main
  const mainAddr = expectInIndirectReceipt(
    receipt,
    deployer.interface,
    "RTokenCreated"
  ).args.main;
