...
export const getDeploymentFilename = (
  chainId: number,
  suffix: string = tempFileSuffix
): string => {
  const path = `${pathToFolder}${chainId}${suffix}`;
  if (!fileExists(path)) fs.writeFileSync(path, JSON.stringify({}, null, 4));
...
export const getRTokenDeploymentFilename = (
  chainId: number,
  name: string
): string => {
...
export const getDeploymentFile = (
  ...
  if (!fileExists(path)) {
    throw new Error(
      `Deployment file ${path} does not exist. Maybe contracts weren't deployed?`
    );
  }
  try {
    ...
    throw new Error(
      `Failed to read ${path}. Maybe the file is badly generated?`
    );
  }
};
