...
async function main() {
  ...
  // Oracles
  const rmavOracle = await ethers.getContractAt(
    "MockAggregatorV3",
    deployments.rMAV_ORACLE
  );
  const rfraOracle = await ethers.getContractAt(
    "MockAggregatorV3",
    deployments.rFRA_ORACLE
  );
  ...
  const deltaOpen = await ethers.getContractAt(
    "DeltaOpen",
    deployments.DELTA_OPEN
  );
  ...
  const rewardControl = await ethers.getContractAt(
    "RewardControl",
    deployments.REWARD_CONTROL
  );

  // Set Reward control
  await deltaOpen
    .connect(sender)
    .setRewardControlAddress(rewardControl.address, { nonce: nonce + i++ });
  console.debug("Set Reward Control");

  await chainlink
    .connect(sender)
    .addAsset(rMAV, rmavOracle.address, { nonce: nonce + i++ });
  console.debug("Added asset rMAV");

  await chainlink
    .connect(sender)
    .addAsset(rFRA, rfraOracle.address, { nonce: nonce + i++ });
  ...
  await deltaOpen
    .connect(sender)
    ._supportMarket(rMAV, deployments.RATE_MODEL, { nonce: nonce + i++ });
  console.debug("Supported market rMAV");

  await deltaOpen
    .connect(sender)
    ._supportMarket(rFRA, deployments.RATE_MODEL, { nonce: nonce + i++ });
