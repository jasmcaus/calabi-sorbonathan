...
async function main() {
  ...
  const TradingLibFactory = await ethers.getContractFactory(
    "RecollateralizationLibP1"
  );
  tradingLib = <RecollateralizationLibP1>(
    await TradingLibFactory.connect(burner).deploy()
  );
  await tradingLib.deployed();
  deployments.tradingLib = tradingLib.address;

  // Deploy RewardableLib external library
  const RewardableLibFactory = await ethers.getContractFactory(
    "RewardableLibP1"
  );
