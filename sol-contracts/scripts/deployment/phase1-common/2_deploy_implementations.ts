...
async function main() {
  ...
  if (!deployments.tradingLib || !deployments.rewardableLib) {
    throw new Error(
      `Missing pre-requisite addresses in network ${hre.network.name}`
    );
  } else if (!(await isValidContract(hre, deployments.tradingLib))) {
    throw new Error(
      `TradingLib contract not found in network ${hre.network.name}`
    );
  } else if (!(await isValidContract(hre, deployments.rewardableLib))) {
    throw new Error(
      `RewardableLib contract not found in network ${hre.network.name}`
    );
  }
  ...
  // 1. ******* Asset Registry ********/
  const AssetRegImplFactory = await ethers.getContractFactory(
    "AssetRegistryP1"
  );
  assetRegImpl = <AssetRegistryP1>(
    await AssetRegImplFactory.connect(burner).deploy()
  );
  ...
  // 2. ******* Backing Manager ***********/
  const BackingMgrImplFactory = await ethers.getContractFactory(
    "BackingManagerP1",
    {
      libraries: {
  ...
  backingMgrImpl = <BackingManagerP1>(
    await BackingMgrImplFactory.connect(burner).deploy()
  );
  ...
  // 3. ********* Basket Handler *************/
  const BskHandlerImplFactory = await ethers.getContractFactory(
    "BasketHandlerP1"
  );
  bskHndlrImpl = <BasketHandlerP1>(
    await BskHandlerImplFactory.connect(burner).deploy()
  );
  ...
  distribImpl = <DistributorP1>(
    await DistribImplFactory.connect(burner).deploy()
  );
  ...
  const RevTraderImplFactory = await ethers.getContractFactory(
    "RevenueTraderP1",
    {
      libraries: { RewardableLibP1: deployments.rewardableLib },
    }
  );
  revTraderImpl = <RevenueTraderP1>(
    await RevTraderImplFactory.connect(burner).deploy()
  );
