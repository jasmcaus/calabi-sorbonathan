...
async function main() {
  ...
  if (!deployments.rsrAsset) {
    throw new Error(
      `Missing deployed contracts in network ${hre.network.name}`
    );
  } else if (!(await isValidContract(hre, deployments.rsrAsset))) {
    throw new Error(
      `RSR Asset contract not found in network ${hre.network.name}`
    );
  }
  ...
  const FacadeWriteLibFactory = await ethers.getContractFactory(
    "FacadeWriteLib"
  );
  ...
  facadeWrite = <FacadeWrite>(
    await FacadeWriteFactory.connect(burner).deploy(deployments.deployer)
  );
