...
async function main() {
  ...
  console.log(
    `Creating Deployment file for network ${hre.network.name} (${chainId})`
  );
  ...
  if (!rsrFeedAddr) {
    throw new Error(
      `Missing address for RSR Feed in network ${hre.network.name}`
    );
  } else if (!(await isValidContract(hre, rsrFeedAddr))) {
    throw new Error(
      `RSR Feed contract not found in network ${hre.network.name}`
    );
  }
  ...
  if (!gnosisAddr) {
    throw new Error(
      `Missing address for GNOSIS_EASY_AUCTION in network ${hre.network.name}`
    );
