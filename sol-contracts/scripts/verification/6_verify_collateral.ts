...
async function main() {
  ...
  if (developmentChains.includes(hre.network.name)) {
    throw new Error(
      `Cannot verify contracts for development chain ${hre.network.name}`
    );
  }

  const assetCollDeploymentFilename = getAssetCollDeploymentFilename(chainId);
  deployments = <IAssetCollDeployments>(
    getDeploymentFile(assetCollDeploymentFilename)
  );

  let asset = await ethers.getContractAt(
    "Asset",
    deployments.collateral.DAI as string
  );
  ...
  const aTokenCollateral: ATokenFiatCollateral = <ATokenFiatCollateral>(
    await ethers.getContractAt(
      ...
      deployments.collateral.aDAI as string
    )
  );
  ...
  asset = await ethers.getContractAt(
    "Asset",
    aTokenCollateral.address as string
  );
  await verifyContract(
    chainId,
  ...
  asset = await ethers.getContractAt(
    "Asset",
    deployments.collateral.cDAI as string
  );
  await verifyContract(
    chainId,
  ...
  asset = await ethers.getContractAt(
    "Asset",
    deployments.collateral.cWBTC as string
  );
  await verifyContract(
    chainId,
  ...
  asset = await ethers.getContractAt(
    "Asset",
    deployments.collateral.cETH as string
  );
  await verifyContract(
    chainId,
  ...
  asset = await ethers.getContractAt(
    "Asset",
    deployments.collateral.WBTC as string
  );
  await verifyContract(
    chainId,
  ...
  asset = await ethers.getContractAt(
    "Asset",
    deployments.collateral.WETH as string
  );
  await verifyContract(
    chainId,
  ...
  asset = await ethers.getContractAt(
    "Asset",
    deployments.collateral.EURT as string
  );
  await verifyContract(
    chainId,
