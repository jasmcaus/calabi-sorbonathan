...
async function main() {
  ...
  const phase1Deployments = <IDeployments>(
    getDeploymentFile(getDeploymentFilename(chainId))
  );
  const assetCollDeploymentFilename = getAssetCollDeploymentFilename(chainId);
  const assetDeployments = <IAssetCollDeployments>(
    getDeploymentFile(assetCollDeploymentFilename)
  );
  if (!phase1Deployments || !assetDeployments)
    throw new Error("Missing deployments");

  console.log(
    `Starting full verification on network ${hre.network.name} (${chainId})`
  );
