...
async function main() {
  ...
  with deployer account: ${deployerUser.address}`)
    ...
    // ******************** Deploy RToken ****************************************/
    const facadeWrite = <FacadeWrite>await ethers.getContractAt("FacadeWrite", rTokenDeployments.facadeWrite)
    const deployer = <DeployerP1>await ethers.getContractAt("DeployerP1", await facadeWrite.deployer())
