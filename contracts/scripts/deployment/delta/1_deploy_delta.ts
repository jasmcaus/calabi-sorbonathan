...
async function main() {
    ...
    const insurance = await DeltaInsurance.deploy(deployments.MOCK_AAVE, deployments.MOCK_ADAI, deployments.MOCK_ICY, { nonce: nonce + i++ })
    ...
    const USDC = await MockERC20.deploy("USDC", "USDC", 6, { nonce: nonce + i++ })
    const USDT = await MockERC20.deploy("USDT", "USDT", 6, { nonce: nonce + i++ })
