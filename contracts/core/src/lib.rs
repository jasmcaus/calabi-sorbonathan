...
impl ILendingPool for LendingPool {
    fn supply(
        ...
        // Extract tokens
        token::Client::new(&env, &asset).transfer(&from, &env.current_contract_address(), &(amount as i128));
...
fn __accrue_interest(env: &Env, asset: &Address) -> (u128, u128, u128, u64) {
    ...
    if current_rate_info.latest_timestamp == timestamp {
        ...
        return (_interest_earned, _fees_amount, _fees_share, new_rate.try_into().unwrap());
    }
    ...
    if asset_vault.total_borrow.shares == 0 {
        ...
        let utilization = (asset_vault.total_borrow.amount * RATE_PRECISION) / asset_vault.total_asset.amount;
        ...
        _interest_earned =
                (time_delta *
                    asset_vault.total_borrow.amount *
                    current_rate_info.rate_per_sec) /
        ...
        if (current_rate_info.fee_to_protocol_rate > 0) {
            _fees_amount =
                (_interest_earned * current_rate_info.fee_to_protocol_rate) /
                PRECISION;

            _fees_share =
                (_fees_amount * asset_vault.total_asset.shares) /
    ...
    (_interest_earned, _fees_amount, _fees_share, new_rate.try_into().unwrap())
}
...
fn __calculate_interest_rate(interest_rate_info: InterestRateInfo, utilization: u128) -> u128 {
    ...
    if utilization <= optimal_utilization  {
        ...
        let excess_utilization_rate = (utilization_delta * RATE_PRECISION) / (RATE_PRECISION - optimal_utilization);
        new_rate_per_sec =
            base_rate +
            slope_1 +
            (excess_utilization_rate * slope_2) /
            RATE_PRECISION;
    }
