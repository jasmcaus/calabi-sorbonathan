...
export function calculateApy(recentRate: ApyRate, lastRate: ApyRate): [number, number] {
    ...
    const priceGrowth = ((recentRate.basketRate - lastRate.basketRate) / lastRate.basketRate) * 100
    const stGrowth = ((recentRate.rsrExchangeRate - lastRate.rsrExchangeRate) / lastRate.rsrExchangeRate) * 100
...
export function hasAllowance(allowances: BigNumberMap, requiredAllowances: BigNumberMap) {
    return Object.keys(requiredAllowances).every(
        (address) => BigNumber.isBigNumber(allowances[address]) && allowances[address].gte(requiredAllowances[address])
    )
}
...
export const formatUsdCurrencyCell = ({ cell }: { cell: any }) => `$${formatCurrency(+cell.value)}`
