...
export const isValidBasketAtom = atom((get): [boolean, string[]] => {
    ...
    for (const targetUnit of units) {
        const distribution = basket[targetUnit].distribution.reduce((acc, n) => acc + Number(n), 0)
        if (distribution !== 100) {
...
export const addBackupCollateralAtom = atom(null, (get, set, collaterals: Collateral[]) => {
    ...
    for (const unit of Object.keys(collateralByTarget)) {
        const unitCollaterals = [...collateralByTarget[unit], ...(basket[unit]?.collaterals ?? [])]
...
export const addBasketCollateralAtom = atom(null, (get, set, collaterals: Collateral[]) => {
    ...
    for (const unit of Object.keys(collateralByTarget)) {
        ...
        const distribution = new Array(unitCollaterals.length - 1).fill(truncateDecimals(100 / unitCollaterals.length))
...
export const updateBackupBasketUnitAtom = atom(null, (get, set, [unit, data]: [string, BackupUnitBasket]) => {
    const basket = { ...get(backupCollateralAtom) }
...
export const updateBasketUnitAtom = atom(null, (get, set, [unit, data]: [string, PrimaryUnitBasket]) => {
    const basket = { ...get(basketAtom) }
