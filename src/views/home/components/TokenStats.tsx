...
const TokenStats = (props: BoxProps) => {
    ...
    const metrics = useMemo(() => {
        if (data) {
            const rsvMarket = +formatEther(data.token?.totalSupply || "0") * (+data.token?.lastPriceUSD || 0)
            const rsvVolume =
                +formatEther(data.token?.cumulativeVolume || "0") * (+data.token?.lastPriceUSD || 0) +
                rpayOverview.volume

            return {
                totalValueLockedUSD: `$${formatCurrency((+data.protocol?.totalValueLockedUSD || 0) + rsvMarket)}`,
                totalRTokenUSD: `$${formatCurrency((+data.protocol?.totalRTokenUSD || 0) + rsvMarket)}`,
                cumulativeVolumeUSD: `$${formatCurrency(rsvVolume + (+data.protocol?.cumulativeVolumeUSD || 0))}`,
                cumulativeRTokenRevenueUSD: `$${formatCurrency(+data.protocol?.cumulativeRTokenRevenueUSD || 0)}`,
                cumulativeInsuranceRevenueUSD: `$${formatCurrency(+data.protocol?.cumulativeInsuranceRevenueUSD || 0)}`,
                transactionCount: formatCurrency(
                    rpayOverview.txCount + (+data.token?.transferCount || 0) + (+data.protocol?.transactionCount || 0)
                ),
                dailyTransactionCount: formatCurrency(
                    rpayOverview.dayTxCount + (+data.usageMetricsDailySnapshots[0]?.dailyTransactionCount || 0)
                ),
                dailyVolume: `$${formatCurrency(
                    rpayOverview.dayVolume + (+data.financialsDailySnapshots[0]?.dailyVolumeUSD || 0)
                )}`,
            }
        }
