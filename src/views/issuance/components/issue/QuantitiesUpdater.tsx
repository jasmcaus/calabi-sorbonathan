 ...
 */
const QuantitiesUpdater = ({ amount, onChange }: { amount: string; onChange(quantities: BigNumberMap): void }) => {
        ...
        async (value: string) => {
            try {
                ...
                if (facadeContract && rToken && Number(value) > 0) {
                    ...
                    const quoteResult = await facadeContract.callStatic.issue(rToken.address, issueAmount)
                    onChange(
