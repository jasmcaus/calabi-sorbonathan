...
export const WithdrawModal = () => {
    ...
    async function callWithdraw() {
        setLoading(true)
        withdraw(account!, lendAsset, +withdrawAmount)
            .then(() => {
    ...
    return (
        <>
            <MaxIssuableUpdater />
            <Modal title={`Withdraw ${lendAsset}`} style={{ width: 400 }} onClose={onClose}>
                {hasExistingBorrow && (
                    <Box mt={5} sx={{ textAlign: "center", fontSize: 1 }}>
                        <Text>
                            <Trans>Repay your borrowed {lendAsset} first to be able to withdraw.</Trans>
                        </Text>
                    </Box>
                )}
