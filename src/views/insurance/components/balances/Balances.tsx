...
const AvailableBalance = () => {
    ...
    useEffect(() => {
        if (claiming && claimTx && ![TRANSACTION_STATUS.SIGNING, TRANSACTION_STATUS.PENDING].includes(claimTx.status)) {
            setClaiming("")
        }
    }, [claimTx, claiming])
...
const StakeBalance = () => {
    ...
    return (
        <Box p={4}>
            ...
            <TokenBalance symbol={rToken?.stToken?.symbol ?? ""} logoSrc="/svgs/strsr.svg" balance={balance} />
            <TokenBalance mt={2} symbol="RSR Value" logoSrc="/svgs/equals.svg" balance={balance * rate} />
            <TokenBalance symbol="USD" logoSrc="/svgs/equals.svg" usd balance={balance * rate * rsrPrice} mt={2} />
        </Box>
    )
}
...
const RSRBalance = () => {
    ...
    return (
        <Box p={4}>
            ...
            <TokenBalance symbol="RSR" balance={balance} />
            <TokenBalance logoSrc="/svgs/equals.svg" symbol="USD" usd balance={balance * rsrPrice} mt={2} />
        </Box>
    )
}
