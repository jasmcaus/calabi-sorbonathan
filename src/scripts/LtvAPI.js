...
const LtvAPI = {
    ...
    getInstance: async function () {
        if (this.ltv != null && this.priceFeed != null)
            return {
                ltv: this.ltv,
    ...
    getCollateralAmount: async function (principalToken, collateralToken, principalAmount, userAddress) {
        const instance = await this.getInstance()
        if (instance == null) return null

        try {
            let principalPriceInUSD = await instance.priceFeed.amountInUSD(principalToken, principalAmount)

            let ltv = await instance.ltv.getRelativeLTV(userAddress, convert(principalPriceInUSD, "wei", "wei"))
