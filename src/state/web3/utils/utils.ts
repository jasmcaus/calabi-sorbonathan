import { DELTA_OPEN, rFRA, rMAV, DELTA_INSURANCE, FAUCET } from "utils/addresses"
import Web3 from "web3"

let web3: Web3 | undefined

const MockERC20 = require("abis/contracts/mocks/MockERC20.sol/MockERC20.json")
...
const Faucet = require("abis/contracts/helpers/Faucet.sol/Faucet.json")
...
export async function getBalance(account: string, tokenAddress: string, decimals: number, str: string = "") {
    if (!account) return 0

    setup()

    const contract = new web3!.eth.Contract(MockERC20.abi, tokenAddress)
...
export async function getAllowance(account: string, spender: string, asset: string) {
    const contract = new web3!.eth.Contract(MockERC20.abi, asset)
...
export async function getLiquidity(asset: string) {
    return getBalance(DELTA_OPEN, asset, 18)
}
...
export async function drip(account: string, amount: number) {
    if (!account) return
    setup()

    const value = toDecimals(amount.toString())

    // console.debug("Deets:", {account, amount, value})

    const contract = new web3!.eth.Contract(Faucet.abi, FAUCET)
    const receipt = await contract.methods.drip([rMAV, rFRA], [value, value], account).send({ from: account })
    console.debug("Receipt:", receipt)
