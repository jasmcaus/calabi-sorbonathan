...
import { ERC20Interface, FacadeInterface, MainInterface, RTokenInterface } from "register-abis"
...
const ReserveTokenUpdater = () => {
    ...
    const getBackingDistribution = useCallback(async (tokenAddress: string, provider: Web3Provider) => {
        try {
            const callParams = {
                abi: FacadeInterface,
            ...
            const [{ erc20s, uoaShares, targets }, { backing, insurance }] = await promiseMulticall(
                [
                    {
                        ...callParams,
                ...
                erc20s.reduce(
                    (acc: any, current: any, index: any) => ({
                        ...
                        [current]: {
                            ...
                            targetUnit: ethers.utils.parseBytes32String(targets[index]).toUpperCase(),
                        },
                    }),
                    {}
                )
    ...
    const getUserRole = useCallback(async (provider: Web3Provider, mainAddress: string, accountAddress: string) => {
        try {
            const callParams = {
                abi: MainInterface,
                address: mainAddress,
