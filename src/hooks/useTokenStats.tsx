...
const useTokenStats = (rTokenId: string, isRSV = false): TokenStats => {
    ...
    useEffect(() => {
        if (data?.rtoken || data?.token) {
            ...
            const dailyVolume = +formatEther(data?.token.dailyTokenSnapshot[0]?.dailyVolume ?? "0")
            const supplyUsd = isRSV ? supply * +data?.token.lastPriceUSD : supply * rTokenPrice
            const volumeUsd = isRSV ? cumulativeVolume * +data?.token.lastPriceUSD : cumulativeVolume * rTokenPrice
            ...
            if (isRSV) {
                ...
                tokenData.cumulativeVolumeUsd = `$${formatCurrency(volumeUsd + rpayOverview.volume)}`
                ;(tokenData.dailyVolume = `$${formatCurrency(dailyVolume + rpayOverview.dayVolume)}`),
                    (tokenData.dailyTransferCount = tokenData.dailyTransferCount + rpayOverview.dayTxCount)
            }
