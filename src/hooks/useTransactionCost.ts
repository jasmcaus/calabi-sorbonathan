...
export const useTransactionGasFee = (
    ...
    const fetchTxFees = useCallback(async (provider: Web3Provider, txs: TransactionState[], account: string) => {
        ...
        try {
            const result = await Promise.all(
                txs.map(async (tx) => {
                    const contract = getContract(tx.call.address, abis[tx.call.abi], provider as Web3Provider, account)

                    const estimate = await contract.estimateGas[tx.call.method](...tx.call.args)
