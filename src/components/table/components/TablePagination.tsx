...
const runtimeTemplate = (str: string, obj: Record<string, any>) => str.replace(/\${(.*?)}/g, (x, g) => obj[g])
                ...
                {(canPreviousPage || canNextPage) && (
                    <Box variant="table.pagination.navigation" sx={{ display: "flex" }}>
                        <Box variant="table.pagination.navigation.button" mr={1}>
                            <SmallButton variant="muted" onClick={() => gotoPage(0)} disabled={!canPreviousPage}>
                                {"<<"}
                            </SmallButton>
                        </Box>
                        <Box variant="table.pagination.navigation.button" mr={1}>
                            <SmallButton variant="muted" onClick={() => previousPage()} disabled={!canPreviousPage}>
                                {"<"}
                            </SmallButton>
                        </Box>
                        <Box variant="table.pagination.navigation.button" mr={1}>
                            <SmallButton variant="muted" onClick={() => nextPage()} disabled={!canNextPage}>
                                {">"}
                            </SmallButton>
                        </Box>
