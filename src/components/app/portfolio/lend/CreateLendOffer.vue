    ...
    <main>
        <div class="header">
            <div class="toolbar">
                ...
                <PrimaryButton
                    :progress="approving"
                    v-if="principalAmount <= 0 || !checkbox || collateralTokens.length == 0"
                    :width="'160px'"
                    :text="'Create'"
                    :state="'disable'"
                />
                ...
                <PrimaryButton
                    :state="creating ? 'disable' : ''"
                    :progress="creating"
                    v-else-if="Number($fromWei(allowance)) >= Number(safePrincipal())"
                    v-on:click="createPopUp = true"
                    :text="'Create'"
                    :width="'160px'"
                />
                ...
                <PrimaryButton
                    ...
                    :text="`Approve ${$findAsset(principalToken).symbol}`"
                    v-on:click="approve()"
                    :width="'200px'"
                />
            </div>
                ...
                <div class="form">
                    <div class="option">
                        <div>
                            <p>Principal</p>
                            <p>
                                Bal : <span>{{ findTokenBalance() }}</span>
                            </p>
                        </div>
                        <div>
                            ...
                            <div class="click_1" v-on:click="dropDown = !dropDown">
                                ...
                                <div class="drop_down" v-show="dropDown">
                                    <div
                                        class="drop_item"
                                        v-for="asset in $otherAssets(principalToken)"
                                        :key="asset.id"
                    ...
                    <div class="choose_col">
                        <div>
                            ...
                            <p>
                                <span>{{ collateralTokens.length }}</span
                                >/3
                            </p>
                        </div>
                        <div class="tokens">
                            <div
                                v-for="asset in $findConjugates(principalToken)"
                                :class="collateralTokens.includes(asset.address) ? 'active border' : 'border'"
                                :key="asset.id"
                                v-on:click="toggleToken(asset.address)"
                            >
                                <div class="token">
                        ...
                        <div>
                            <div class="input">
                                <input
                                    type="number"
                                    disabled
                                    :style="getInputWidth(daysToExpire)"
                                    placeholder="0"
                                    min="0"
